...
sUserData_t DFRobot_3DFace::getFaceMassage(void)
{
  sUserData_t data;
  uint16_t len = 0;
...
int16_t DFRobot_3DFace_I2C::readReg(uint8_t reg,uint8_t *data,uint8_t len)
{
  ...
  DFRobot_3DFace_UART::DFRobot_3DFace_UART(SoftwareSerial *sSerial, uint16_t Baud)
  {
...
#else
  DFRobot_3DFace_UART::DFRobot_3DFace_UART(HardwareSerial *hSerial, uint16_t Baud ,uint8_t txpin, uint8_t rxpin)
  {
